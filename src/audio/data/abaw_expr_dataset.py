...
class AbawExprDataset(Dataset):
    ...
    def parse_features(
        self, lab_feat_df: pd.core.frame.DataFrame, lab_filename: str
        ...
        for dp, _, fps in os.walk(self.labels_root):
            ...
            for fp in fps:
                ...
                features = pd.read_csv(
                    os.path.join(self.features_root, fp.replace("txt", "csv")),
                    sep=",",
                ...
                timings, expr_processed_labs = self.parse_features(
                    lab_feat_df=labs_and_feats, lab_filename=fp
                )
...
if __name__ == "__main__":
    ...
    dl = torch.utils.data.DataLoader(aed, batch_size=8, shuffle=False, num_workers=8)
