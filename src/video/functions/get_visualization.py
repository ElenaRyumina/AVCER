...
class VideoCamera(object):
    def __init__(
        self, path_video="", path_report="", path_save="", name_labels="", conf=0.7
    ):
    ...
    def draw_prob(self, emotion_yhat, best_n, startX, startY, endX, endY):
        ...
        label = "{}: {:.2f}%".format(
            self.labels[best_n[0]], emotion_yhat[best_n[0]] * 100
        )
        ...
        cv2.putText(
            ...
            label,
            (
                center_face - round(text_width_2 / 2),
                p1[1] - round(((p2[0] - p1[0]) * 20) / 360),
            ),
            self.font,
            lw / 3,
            text_fond,
    ...
    def get_video(self):
        ...
        vid_writer = cv2.VideoWriter(
            self.path_save, cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h)
        )
        while True:
