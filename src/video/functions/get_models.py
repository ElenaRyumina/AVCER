...
def load_weights_EE(path):
    ...
    EE_AN_model = tf.keras.models.Model(
        inputs=EE_AN_model.input, outputs=[EE_AN_model.get_layer("features").output]
    )
    return EE_AN_model
...
def LSTM():
    ...
    X = tf.keras.layers.Masking(mask_value=0.0)(input_lstm)
    X = tf.keras.layers.LSTM(
        512, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(1e-3)
    )(X)
    X = tf.keras.layers.Dropout(rate=0.2)(X)
    X = tf.keras.layers.LSTM(
        256, return_sequences=False, kernel_regularizer=tf.keras.regularizers.l2(1e-3)
    )(X)
    X = tf.keras.layers.Dropout(rate=0.2)(X)
